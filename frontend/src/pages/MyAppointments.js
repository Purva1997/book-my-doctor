...
function MyAppointments () {
        ...
        const options = {
            ...
            handler: async (response) => {
                ...
                try {
                    ...
                    console.log("Error for payment to the doctor:", error)
    ...
    const appointmentRazorpay = async (appointmentId) => {
        ...
        try {
            ...
            console.error("Error for payment to the doctor:", error.message)
        }
                ...
                {appointments.slice(0, 10).map((item, index) => (
                    <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b" key={index}>
                        ...
                        <div className="flex-1 text-sm text-zinc-600">
                            ...
                            <p className="text-xs mt-1"><span className="text-zinc-700 text-sm font-medium">Date & Time:</span> {slotDateFormate(item.slotDate)} | {item.slotTime}</p>
                        </div>
                        <div></div>
                        <div className="flex flex-col gap-2 justify-center">
                            {!item.cancelled && item.payment && !item.isCompleted && <button className="sm:min-w-48 py-2 border rounded bg-blue-100 text-stone-500">Paid</button>}
